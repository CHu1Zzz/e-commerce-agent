...
class PIIScrubber:
    ...
    def redact(self, text: str) -> str:
        ...
        # 15位身份证
        result = re.sub(r"\d{15}", lambda m: m.group()[:6] + "*****" + m.group()[-4:], result)
        ...
        # 邮箱脱敏
        result = re.sub(
            r"([a-zA-Z0-9._%+-]{3})[a-zA-Z0-9._%+-]*@([a-zA-Z0-9.-]{3})\.[a-zA-Z]{2,}",
        ...
        # 地址脱敏（只保留省市区，隐藏详细门牌号）
        result = re.sub(
            r"((?:[一-龥]+(?:省|市|区|县)){1,2}(?:[一-龥]+区)?)([一-龥]+(?:街|路|弄|号|栋|单元|室).*)",
            r"\1[详细地址已脱敏]",
            result,
        )
        result = re.sub(
            r"([一-龥]+\d+号(?:[一-龥]?\d+号)?)([一-龥]*.*)",
            r"\1[详细信息已脱敏]",
            result,
        )
